...
import {
  UserRepository, VendorRepository, ContractRepository,
  ReminderRepository, ActivityEventRepository
} from './repositories';
...
export async function seedDatabase(): Promise<void> {
  try {
    ...
    const contractManagerUser = await userRepo.createUser(
      'manager@example.com',
      'Contract Manager',
      'Contract Manager',
      'auth0|manager-user-id'
    );

    const viewerUser = await userRepo.createUser(
      'viewer@example.com',
      'Viewer User',
      'Viewer',
      'auth0|viewer-user-id'
    );
