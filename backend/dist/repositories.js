...
exports.DocumentMetadataRepository = exports.ActivityEventRepository = exports.ReminderRepository = exports.ContractRepository = exports.VendorRepository = exports.UserRepository = exports.Repository = void 0;
...
class Repository {
    ...
    async create(entity) {
        ...
        await database_1.database.run(sql, values);
        return entity;
    }
    async findById(id) {
        const sql = `SELECT * FROM ${this.tableName} WHERE ${this.keyField} = ?`;
        const row = await database_1.database.get(sql, [id]);
        return row ? this.fromRow(row) : null;
    }
    async findAll(limit = 100, offset = 0) {
        const sql = `SELECT * FROM ${this.tableName} LIMIT ? OFFSET ?`;
        const rows = await database_1.database.all(sql, [limit, offset]);
...
class UserRepository extends Repository {
    ...
    async findByEmail(email) {
        ...
        const row = await database_1.database.get(sql, [email]);
        return row ? this.fromRow(row) : null;
    }
    async findByAuthSubjectId(authSubjectId) {
        const sql = 'SELECT * FROM users WHERE auth_subject_id = ?';
        const row = await database_1.database.get(sql, [authSubjectId]);
        return row ? this.fromRow(row) : null;
    }
    async createUser(email, displayName, role, authSubjectId) {
...
class VendorRepository extends Repository {
    ...
    async createVendor(legalName, riskTier, status = 'Active') {
        const now = new Date().toISOString();
        const vendor = {
            id: (0, crypto_1.randomUUID)(),
    ...
    async searchByName(query) {
        ...
        const pattern = `%${query}%`;
        const rows = await database_1.database.all(sql, [pattern, pattern]);
        return rows.map(row => this.fromRow(row));
    }
    async findByLegalNameNormalized(legalName) {
        const sql = 'SELECT * FROM vendors WHERE lower(trim(legal_name)) = lower(trim(?))';
        const row = await database_1.database.get(sql, [legalName]);
        return row ? this.fromRow(row) : null;
    }
}
...
class ContractRepository extends Repository {
    ...
    async findByVendor(vendorId) {
        ...
        const rows = await database_1.database.all(sql, [vendorId]);
        return rows.map(row => this.fromRow(row));
    }
    async searchByTitle(query) {
        const sql = 'SELECT * FROM contracts WHERE title LIKE ? AND archived = 0';
        const pattern = `%${query}%`;
        const rows = await database_1.database.all(sql, [pattern]);
        return rows.map(row => this.fromRow(row));
    }
    async findExpiringContracts(daysAhead) {
        ...
        const rows = await database_1.database.all(sql, [now, futureDate]);
        return rows.map(row => this.fromRow(row));
    }
    async findExpiredContracts() {
        const now = new Date().toISOString().split('T')[0];
        const sql = 'SELECT * FROM contracts WHERE archived = 0 AND end_date < ?';
        const rows = await database_1.database.all(sql, [now]);
        return rows.map(row => this.fromRow(row));
    }
}
...
class ReminderRepository extends Repository {
    ...
    async findByContract(contractId) {
        ...
        const rows = await database_1.database.all(sql, [contractId]);
        return rows.map(row => this.fromRow(row));
    }
...
class ActivityEventRepository extends Repository {
    constructor() {
        super(...arguments);
        this.tableName = 'activity_events';
    }
    toRow(event) {
        return {
            id: event.id,
    ...
    async findByEntity(entityType, entityId) {
        ...
        const rows = await database_1.database.all(sql, [entityType, entityId]);
        return rows.map(row => this.fromRow(row));
    }
    async logActivity(userId, entity, entityId, action, summary) {
        const now = new Date().toISOString();
        const event = {
            id: (0, crypto_1.randomUUID)(),
            timestamp: now,
...
class DocumentMetadataRepository extends Repository {
    constructor() {
        super(...arguments);
        this.tableName = 'document_metadata';
    }
    toRow(doc) {
        return {
            id: doc.id,
...
exports.DocumentMetadataRepository = DocumentMetadataRepository;
